...
However, we don't know the directory where our file was uploaded. So we would need to perform `directory fuzzing`. There is a variety of tools that can help you with that such as: [gobuster](), [wfuzz](), [fuff]() or [dirsearch](). Here I skiped that part and proceeded to navitage to the directory instead.
...
To get a reverse shell, we would just need to execute ur binary and listen in the attack host with [nc](). For Windows machines, it is better to use [rlwrap]() before the command.
...
There is a suite of tools that abuse of this privilege. In this case I am going to use [PrintSpoofer]().
...
Now as administrators, we can perform a variety of attacks, such as: Dump LSASS, LSA, SAM and so on. Doing it remotely is far more easier than loading [Mimikiatz](), with some modules we do not to be worried about getting detected. However, a variety of EDRs already know this method, so you wouldn't be able to use intrusive modules. You would need to find a workaround and modify such module.
